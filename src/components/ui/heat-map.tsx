...
const Box = ({ children, flexDirection = 'column', gap, marginTop }: any) => (
  <div style={{ display: 'flex', flexDirection, gap: gap ? `${gap * 8}px` : undefined, marginTop: marginTop ? `${marginTop * 8}px` : undefined }}>{children}</div>
);
const Text = ({ children, color }: any) => (
  <span style={{ color, whiteSpace: 'pre', fontFamily: 'monospace' }}>{children}</span>
);
